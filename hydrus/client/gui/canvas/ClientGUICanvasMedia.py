...
class MediaContainer( QW.QWidget ):
    ...
    def _ApplyEdgeLighting( self ):
        
        if not hasattr( self, '_edge_lighting' ):
            
            self._edge_lighting = ClientGUICanvasMediaEffects.EdgeLightingWidget( self )
            self._edge_lighting.lower()
            print(self._edge_lighting)
            
        
        if self._media_window is not None:
            
            self._edge_lighting.lower()
            self._edge_lighting.stackUnder( self._media_window )
            
            self._edge_lighting.SetGeometryFromRect(
                self._media_window.geometry()
            )
            
        
    
    def _DisableEdgeLighting( self ):
    ...
    def _ApplySlideshowTransition( self, old_media, new_media, old_pixmap ):
            ...
            if transition == CC.MEDIA_TRANSITION_EFFECT_RANDOM:
                
                transition_fn = random.choice( tuple( transition_fns.values() ) )
                
            else:
                
                transition_fn = transition_fns.get( transition )
    ...
    def _SizeAndPositionChildren( self ):
        ...
        if self._media is not None:
            ...
            if hasattr( self, '_edge_lighting' ) and self._edge_lighting.isVisible():
    ...
    def SetMedia( self, media: ClientMediaSingle.MediaSingle, maintain_zoom, maintain_zoom_type, maintain_pan, start_paused = None ):
        ...
        old_media_pixmap = None
        if previous_media is not None:
            
            old_media_pixmap = self._GrabCurrentMediaPixmap()
